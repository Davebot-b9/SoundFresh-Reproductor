...
class RegisterUserView(QDialog):
    ...
    def create_user(self):
        ...
        if not (password_sf and confirm_sf and user_sf):
            QMessageBox.warning(self, 'Error', 'Please enter valid data', QMessageBox.StandardButton.Close, QMessageBox.StandardButton.Close)
        elif password_sf != confirm_sf:
            QMessageBox.warning(self, 'Error', 'Passwords do not match', QMessageBox.StandardButton.Close, QMessageBox.StandardButton.Close)
        else:
            try:
                # Validación de usuario y contraseña
                if len(user_sf) > 10:
                    QMessageBox.warning(self, 'Error', 'Username must be up to 10 characters', QMessageBox.StandardButton.Close, QMessageBox.StandardButton.Close)
                elif not (user_sf.isalnum() and user_sf.isascii()):
                    QMessageBox.warning(self, 'Error', 'Username can only contain letters and numbers', QMessageBox.StandardButton.Close, QMessageBox.StandardButton.Close)
                    ...
                    QMessageBox.warning(self, 'Error', 'Password must be 8-20 characters long and include at least one uppercase letter, one digit, and one special character', QMessageBox.StandardButton.Close, QMessageBox.StandardButton.Close)
                else:
                    ...
                    if not (self.name_input.text().isalpha() and self.lastname_input.text().isalpha()):
                        QMessageBox.warning(self, 'Error', 'Name and lastname can only contain letters', QMessageBox.StandardButton.Close, QMessageBox.StandardButton.Close)
                    else:
